...
class LiftSelectorApp(tk.Tk):
    ...
    def generate_graph(self):
        ...
        # Path to the file
        json_file = os.path.join(ROOT, MEASUREMENT_DATA_ANALYSIS, 'DOOR OPENING AND CLOSING CYCLE COUNT', f'DC-{self.selected_lift}.json')
        ...
        fig = px.line(
            ...
            x="Date",
            y="Cycles",
            title=f"({self.selected_lift}) Lift Cycles Over Time",
            labels={"Cycles": "Number of Cycles", "Date": "Date"},
            markers=True,
        )
        fig.update_layout(
            xaxis=dict(title="Date", tickangle=45),
            yaxis=dict(title="Number of Cycles"),
            hovermode="x unified",
        )
        fig.show()
