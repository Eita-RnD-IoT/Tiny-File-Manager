...
class LiftSelectorApp(tk.Tk):
    ...
    def generate_graph(self):
        ...
        # Path to the file
        json_file = os.path.join(ROOT, MEASUREMENT_DATA_ANALYSIS, 'MILEAGE', f'M-{self.selected_lift}.json')
        ...
        files_data = data.get("file_mileages", {})
        if not files_data:
        ...
        rows = []
        for file_name, mileage in files_data.items():
        ...
        fig = px.line(
            ...
            x="Start Date",
            y="Mileage",
            title=f"({self.selected_lift}) Lift Mileage Over Time",
            labels={"Mileage": "Mileage (km)", "Start Date": "Start Date"},
            markers=True,
        )
        fig.update_layout(
            xaxis=dict(title="Start Date", tickangle=45),
            yaxis=dict(title="Mileage (km)"),
            hovermode="x unified",
        )
        fig.show()
