...
class LiftSelectorApp(tk.Tk):
    ...
    def generate_graph(self):
        ...
        json_file = os.path.join(ROOT, MEASUREMENT_DATA_ANALYSIS, 'FLOOR COUNT', f'FC-{self.selected_lift}.json')
        ...
        rows = []
        for file_name, floor_data in files_data.items():
            lift_name, date_range = file_name.split("-FROM_")
            date_range = date_range.replace(".csv", "")
            start_date, end_date = date_range.split("_TO_")

            for floor, count in floor_data.items():
                rows.append({
                    "Lift": lift_name,
                    "Floor": int(floor),
                    "Frequency": count,
                    "Start Time": pd.to_datetime(start_date, format="%Y-%m-%d%H%M%S").strftime('%Y-%m-%d %H:%M:%S'),
                    "End Time": pd.to_datetime(end_date, format="%Y-%m-%d%H%M%S").strftime('%Y-%m-%d %H:%M:%S')
                })

        # Create DataFrame and plot graph
        ...
        fig = px.bar(
            ...
            y="Frequency",
            color="Start Time",
            hover_data=["Start Time", "End Time", "Frequency"],
            title=f"({self.selected_lift}) Lift Movements by Floor and Date",
            labels={"Frequency": "Frequency", "Floor": "Floor Number", "Start Time": "Start Date & Time"},
            text_auto=True,
        )
        fig.update_layout(
